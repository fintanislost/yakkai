    ...
    for (const auto& code : codes) {
        ...
        for (auto pb : bindings) {
            ...
            if (b.descriptor_type == SPV_REFLECT_DESCRIPTOR_TYPE_UNIFORM_BUFFER) {
                ...
                auto  blockIt = std::find_if(ref.blocks.begin(),
                                            ref.blocks.end(),
                                            [&](const ShaderReflected::Block& existing) {
                                                return existing.binding == b.binding ||
                                                       existing.name == block_name;
                                            });
                if (blockIt == ref.blocks.end()) {
                    ref.blocks.push_back(ShaderReflected::Block { //.index = i,
                                                                  .binding    = b.binding,
                                                                  .size       = block.size,
                                                                  .name       = block_name,
                                                                  .member_map = {} });
                    blockIt = ref.blocks.end() - 1;
                } else {
                    blockIt->size = std::max(blockIt->size, block.size);
                }
                auto& ref_block = *blockIt;
                ...
                for (u32 i = 0; i < block.member_count; i++) {
                    ...
                    {
                        // bunif.num = GetTypeNum(type);
                        bunif.block_index = static_cast<int>(std::distance(ref.blocks.begin(), blockIt));
...
bool wallpaper::vulkan::CompileAndLinkShaderUnits(std::span<const ShaderCompUnit>  compUnits,
    ...
    LOG_INFO("shader compile: begin units=%zu client_ver=%d", compUnits.size(), static_cast<int>(opt.client_ver));
    glslang::TProgram program;
    EShMessages       emsg;
    SetMessageOptions(opt, emsg);
    std::vector<std::unique_ptr<glslang::TShader>> shaders;
    for (size_t index = 0; index < compUnits.size(); ++index) {
        auto& unit = compUnits[index];
        LOG_INFO("shader compile: creating shader index=%zu stage=%d", index, static_cast<int>(unit.stage));
        shaders.emplace_back(std::make_unique<glslang::TShader>(unit.stage));
        auto& shader = *(shaders.back());
        LOG_INFO("shader compile: parsing shader index=%zu", index);
        if (! parse(unit, opt, emsg, shader)) return false;
        LOG_INFO("shader compile: parsed shader index=%zu", index);
        program.addShader(&shader);
    }

    LOG_INFO("shader compile: linking program");
    if (! program.link(emsg)) {
        LOG_ERROR("glslang(link): %s\n", program.getInfoLog());
        return false;
    }
    LOG_INFO("shader compile: linked program");
    ...
    LOG_INFO("shader compile: mapping IO");
    if (! (program.mapIO(&resolver, &ioMapper))) {
        LOG_ERROR("glslang(mapIo): %s\n", program.getInfoLog());
        return false;
    }
    LOG_INFO("shader compile: IO mapped");
    ...
    for (size_t index = 0; index < compUnits.size(); ++index) {
        auto& unit = compUnits[index];
        LOG_INFO("shader compile: generating SPIR-V index=%zu stage=%d", index, static_cast<int>(unit.stage));
        Uni_ShaderSpv spv = std::make_unique<ShaderSpv>();
        ...
        spvs.emplace_back(std::move(spv));
        LOG_INFO("shader compile: generated SPIR-V index=%zu words=%zu",
                 index,
                 spvs.back()->spirv.size());

        auto messages = logger.getAllMessages();
        if (messages.length() > 0) LOG_ERROR("glslang(spv): %s\n", messages.c_str());
    }

    LOG_INFO("shader compile: complete");
    return true;
}
