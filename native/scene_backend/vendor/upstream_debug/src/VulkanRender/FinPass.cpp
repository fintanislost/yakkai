...
void FinPass::prepare(Scene& scene, const Device& device, RenderingResources& rr) {
    ...
    {
        LOG_INFO("finpass prepare: compiling fullscreen shaders");
        ShaderCompOpt opt;
        ...
        LOG_INFO("finpass prepare: shader compile produced %zu stages", spvs.size());
    }
    ...
    std::vector<VkVertexInputAttributeDescription> attr_descriptions;
    {
        ...
        LOG_INFO("finpass prepare: fullscreen vertex buffer ready");
    }
    ...
    {
        LOG_INFO("finpass prepare: creating render pass");
        ...
        LOG_INFO("finpass prepare: creating graphics pipeline");
        if (! pipeline.create(device, pass, m_desc.pipeline)) return;
        LOG_INFO("finpass prepare: graphics pipeline ready");
    }
    /*
    ...
    */
    ...
    setPrepared();
    LOG_INFO("finpass prepare: complete");
}
