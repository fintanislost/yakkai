...
void addReflectionPass(RenderGraph& rgraph, const TexNode::Desc& in, const TexNode::Desc& out) {
    rgraph.addPass<vulkan::ReflectionPass>(
        "reflection",
        PassNode::Type::Reflection,
        [&in, &out](RenderGraphBuilder& builder, vulkan::ReflectionPass::Desc& desc) {
            auto* in_node  = builder.createTexNode(in);
            auto* out_node = builder.createTexNode(out, true);
            builder.read(in_node);
            builder.write(out_node);
            desc.src = in_node->key();
            desc.dst = out_node->key();
        });
}
...
static void ToGraphPass(SceneNode* node, std::string_view output, i32 imgId, ExtraInfo& extra) {
    ...
    const bool needsReflection =
        std::find(material->textures.begin(), material->textures.end(), WE_REFLECTION_BUFFER) !=
        material->textures.end();

    if (needsReflection && ! extra.reflection_ready) {
        rg::addReflectionPass(rgraph,
                              rg::createTexDesc(std::string(SpecTex_Default)),
                              rg::createTexDesc(std::string(WE_REFLECTION_BUFFER)));
        extra.reflection_ready = true;
    }
