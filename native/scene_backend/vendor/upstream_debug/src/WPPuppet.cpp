...
void WPPuppet::prepared() {
    ...
    for (uint i = 0; i < bones.size(); i++) {
        ...
        if (! b.noParent() && b.parent >= i) {
            LOG_ERROR("puppet prepared: invalid parent index %u for bone %u", b.parent, i);
            combined_tran[i] = Affine3f::Identity();
            b.offset_trans   = Affine3f::Identity();
            continue;
        }
        combined_tran[i] =
    ...
    for (auto& anim : anims) {
        if (anim.fps <= 0.0) {
            LOG_ERROR("puppet prepared: invalid animation fps %.3f for animation %d",
                      anim.fps,
                      anim.id);
...
std::span<const Eigen::Affine3f> WPPuppet::genFrame(WPPuppetLayer& puppet_layer,
    ...
    for (uint i = 0; i < m_final_affines.size(); i++) {
        ...
        if (! bone.noParent() && bone.parent >= i) {
            LOG_ERROR("puppet genFrame: invalid parent index %u for bone %u", bone.parent, i);
            continue;
        }
        ...
        for (auto& layer : puppet_layer.m_layers) {
            ...
            if (layer.anim->bframes_array[i].frames.empty()) {
                LOG_ERROR("puppet genFrame: empty bone frame list for animation %d bone %u",
                          layer.anim->id,
                          i);
                continue;
            }
            ...
            auto&  frames  = layer.anim->bframes_array[i].frames;
            if ((usize)info.frame_a >= frames.size() || (usize)info.frame_b >= frames.size()) {
                LOG_ERROR("puppet genFrame: interpolation frame out of range for animation %d bone %u (%zu frames, a=%zu, b=%zu)",
                          layer.anim->id,
                          i,
                          frames.size(),
                          (usize)info.frame_a,
            ...
            auto&  frame_base = frames[(usize)0];
            auto&  frame_a = frames[(usize)info.frame_a];
            auto&  frame_b = frames[(usize)info.frame_b];
...
WPPuppet::Animation::getInterpolationInfo(double* cur_time) const {
    ...
    if (length <= 0 || frame_time <= 0.0 || max_time <= 0.0) {
        LOG_ERROR("puppet interpolation: invalid animation timing for animation %d", id);
        _info.frame_a = 0;
        _info.frame_b = 0;
        _info.t       = 0.0;
        return _info;
