...
struct WPCameraPathSegment {
    double                         duration { 0.0 };
    std::vector<WPCameraPathKeyframe> keyframes;
};

struct WPCameraPathAnimation {
    std::vector<WPCameraPathSegment> segments;
...
public:
    ...
    void SetPerspectiveCameraPath(const std::shared_ptr<SceneCamera>& camera,
...
private:
    ...
    std::shared_ptr<SceneCamera> m_perspectiveCamera;
    std::shared_ptr<SceneNode>   m_perspectiveCameraNode;
    WPCameraPathAnimation        m_perspectiveCameraPath;
};
} // namespace wallpaper
